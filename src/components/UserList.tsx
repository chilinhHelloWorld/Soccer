...
const UserList = () => {
    const [users, setUser] = useState<user[]>([{ id: 1, Name: "Linh", BoD: "21/01/1999", Gender: true, Address: "Soc Trang" }]);
            ...
            result = currentPost.map((user, index) => {
                ...
                return (
                    ...
                    <tr onClick={handleClick} style={{ cursor: 'pointer' }}>
                        ...
                        <td>{user.BoD}</td>
                        <td>
                            <span className={`label label-${statusClass}`}>{statusGender}</span>
                        </td>
                        <td>{user.Address}</td>
                        <td>
                            {/* <button
    ...
    const showFormSave = () => {
        return showForm ? <AddUser user={users}
            onCloseForm={() => onCloseForm()}
            setUser={setUser}
            userEdit={editUser}
            seteditUser={seteditUser}></AddUser> : "";
    }
